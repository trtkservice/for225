...
def run_grid_search_raptor():
    ...
    print(f"🔎 Raptor225 Micro Simulation (2 Lots, Rakuten Cost)")
    print(f"   Period: 2018-2025 | Logic: Stop 1.0 / Target 2.0")
    print("="*100)
    print("Stop | Tgt  || Ret%   | Avg/Mo% | Avg/Mo(JPY) | PF   | Win%  | Trades")
    print("-" * 100)
    
    for s_mult, t_mult in itertools.product(STOP_RANGE, TARGET_RANGE):
        res = run_raptor_simulation_risk(df_1m, df_15m, BEST_G, BEST_N, s_mult, t_mult, df_daily=df_1m.resample('D').agg({'High':'max','Low':'min','Close':'last'}).dropna().assign(ATR=lambda x: (x['High']-x['Low']).rolling(14).mean()))
